...
if [[ "$1" == apache2* ]] || [ "$1" == php-fpm ]; then
	...
	uniqueEnvs=(
		...
		AUTH_EXTEND_TOKEN_SEND
		AUTH_EXTEND_TOKEN_TEMPLATE
		AUTH_EXTEND_TOKEN_AUTH_A
		AUTH_EXTEND_TOKEN_AUTH_B
		MEDIA_SCAN_SEND
		MEDIA_SCAN_AUTH_A
		MEDIA_SCAN_AUTH_B
...
# JOK - 28-Feb-2018 not an elegant solution, but we need to get these
# booleans set as primitive,

#	WP_DEBUG_DISPLAY,SAVEQUERIES,GTM_ON

		if [ "$WORDPRESS_WP_DEBUG_DISPLAY" ]; then
			set_config 'WP_DEBUG_DISPLAY' 1 boolean
		fi

		if [ "$WORDPRESS_SAVEQUERIES" ]; then
			set_config 'SAVEQUERIES' 1 boolean
		fi
